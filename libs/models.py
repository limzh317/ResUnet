...
class ResUNet1(nn.Module):
    ...
    """
    def __init__(self, is_init_encoder=True, base_net="resnet101"):
        super(ResUNet1, self).__init__()

        self.is_init_encoder = is_init_encoder
        self.encoder = Resnet(base_net)
        self.decoder = Decoder1().cuda()
        self.pretrained_state = torchmodel.resnet101(pretrained=True).state_dict()
    ...
    def init_encoder(self):
        ...
        pretrained_state = {k: v for k, v in self.pretrained_state.items() if
...
class ResUNet2(nn.Module):
    """
    This version have slight difference with ResUNet2.
    shortcut convolution and bn
    middle level supervision
    """
    def __init__(self, is_init_encoder=True, base_net="resnet101"):
        super(ResUNet2, self).__init__()
        print("Using {}!".format(base_net))
        self.is_init_encoder = is_init_encoder
        self.encoder = Resnet(base_net)
        if base_net == 'resnet18':
            self.pretrained_state = torchmodel.resnet18(pretrained=True).state_dict()
        elif base_net == 'resnet34':
            self.pretrained_state = torchmodel.resnet34(pretrained=True).state_dict()
        elif base_net == 'resnet50':
            self.pretrained_state = torchmodel.resnet50(pretrained=True).state_dict()
        elif base_net == 'resnet101':
            self.pretrained_state = torchmodel.resnet101(pretrained=True).state_dict()
        elif base_net == 'resnet152':
            self.pretrained_state = torchmodel.resnet152(pretrained=True).state_dict()
    ...
    def forward(self, feat):
        residual = feat
        x5, x4, x3, x2, x1 = self.encoder(feat)
        # print("x5 shape:{}, x4 shape:{}, x3 shape:{}".format(x5.shape, x4.shape, x3.shape))
        hidden_feature = [x1, x2, x3, x4, x5]
        output = self.decoder(hidden_feature, residual)
